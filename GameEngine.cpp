...
void GameEngine::reset_total_score(QLabel* score_bar) {
    total_score = 0;
    score_bar->setText("Score: 0");
}
...
void GameEngine::spawn_enemy(GameField& game_field) {
    enemy_spawn_timer += 1;
    if (enemy_spawn_timer%ENEMY_SPAWN_INTERVAL == 1) {
        int MAX_HP = 2 + rand()%4;
        int hp = MAX_HP;
    ...
    if(!enemy_list.isEmpty()) {
        for (QList<Enemy*>::iterator enemy = enemy_list.begin(); enemy != enemy_list.end(); /*in body*/) {
            ...
            if (collide((*enemy), player)) {
                ...
                if (unit_collide_delay%UNIT_COLLIDE_DAMAGE_INTERVAL == 1){ // 1 hp deducted for each 0.6s
                    update_hp_bar = true;
                    if(player->take_damage()) {
                        game_field.remove_and_delete_view(player->get_view());
                        player->register_view(nullptr);
                        return;
                    }
                }
            }
